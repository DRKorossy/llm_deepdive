...
class TransformerBlock(nn.Module):
    ...
    def __init__(self, model_dim: int, num_heads: int):
        super().__init__()
        torch.manual_seed(0)
        self.multiheaded_self_attention = self.MultiHeadedSelfAttention(model_dim, num_heads)
        self.vanilla_neural_network = self.VanillaNeuralNetwork(model_dim)
        self.layer_norm_1 = nn.LayerNorm(model_dim)
        self.layer_norm_2 = nn.LayerNorm(model_dim)

    def forward(self, embedded: TensorType[float]) -> TensorType[float]:
        torch.manual_seed(0)
        embedded = embedded + self.multiheaded_self_attention(self.layer_norm_1(embedded))
        embedded = embedded + self.vanilla_neural_network(self.layer_norm_2(embedded))
