...
class GPT(nn.Module):
    ...
    def forward(self, context: TensorType[int]) -> TensorType[float]:
        torch.manual_seed(0)
        embedded = self.word_embeddings(context) + self.positional_embeddings(torch.arange(context.size()[1]))
        transformed = self.transformer_blocks(embedded)
        out = self.final_layer_norm(transformed)
        logits = self.vocab_projection(out)
